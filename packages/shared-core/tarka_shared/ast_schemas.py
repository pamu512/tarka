"""Pydantic AST primitives for transaction rule conditions (legacy Python IR).

Canonical home for in-process AST validation used by pack validators / shadow-test.
Field names include :class:`ingestor.manifest_schema.TransactionSchema` columns plus
extensions such as ``country`` when the ingestion model gains derived fields.

Evaluate path of record remains decision-api / Rust packs — not this IR.
"""
