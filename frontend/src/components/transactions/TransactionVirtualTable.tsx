...
const GRID_COLS =
  "grid grid-cols-[minmax(152px,1.1fr)_minmax(168px,1.2fr)_minmax(168px,1.2fr)_minmax(112px,0.9fr)_minmax(104px,0.8fr)_72px_128px] gap-x-3 items-center px-3";
...
export function TransactionVirtualTable({ data }: { data: readonly TransactionRow[] }): ReactElement {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded-xl border border-surface-700 bg-surface-900/40 overflow-hidden">
      ...
      <div className="border-t border-surface-700 px-3 py-2 text-[11px] text-gray-500 tabular-nums flex justify-between">
        <span>{rows.length.toLocaleString()} rows</span>
        <span>Row height {ROW_HEIGHT_PX}px · virtual overscan 24</span>
      </div>
    </div>
  );
}
