...
class SarFiling(Base):
    ...
    investigative_notes_html: Mapped[str] = mapped_column(Text(), nullable=False, default="")
...
class CaseGraphAnnotation(Base):
    """Per-case graph node annotations (replaces browser localStorage contract)."""

    __tablename__ = "case_graph_annotations"
    __table_args__ = (
        UniqueConstraint("tenant_id", "case_id", name="uq_case_graph_annotations_tenant_case"),
    )
    ...
    updated_by: Mapped[str | None] = mapped_column(String(256), nullable=True, default=None)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
