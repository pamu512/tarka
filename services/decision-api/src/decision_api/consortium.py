...
def hash_entity_id(
    ...
    *,
    ...
    if hash_scope == "tenant":
        payload = f"tenant::{normalized_tenant}::entity::{normalized_entity}".encode(
            "utf-8"
        )
    else:
...
def consortium_score_delta(
    ...
    *,
    ...
    weighted_tenant_score = float(
        data.get("weighted_tenant_score", tenant_count) or tenant_count
    )
    weighted_report_score = float(
        data.get("weighted_report_score", report_count) or report_count
    )
    ...
    raw = (
        (weighted_tenant_score * 4.0)
        + (weighted_report_score * 1.2)
        + (max_severity * 8.0)
    )
    # Penalize consortium signals with high false-positive feedback.
    adjusted = (
        raw
        * max(trust_floor, 1.0 - min(1.0, false_positive_rate))
        * max(trust_floor, min(1.5, quality_score))
    )
