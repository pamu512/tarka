...
"""Hybrid location cohort / co-presence evidence for evaluate audit snapshots."""
...
def build_location_cohort_evidence(
    *,
    ...
    """Audit block when cohort/co-presence signals exist; None when absent (fail soft)."""
    ...
    copresence_block: dict[str, Any] = {}
    if copresence_risk is not None:
        copresence_block["copresence_risk"] = round(copresence_risk, 4)
    if impossible_travel is not None:
        copresence_block["impossible_travel_risk"] = round(impossible_travel, 4)
    if geo_consistency is not None:
        copresence_block["geo_consistency_risk"] = round(geo_consistency, 4)
    if location_confidence is not None:
        copresence_block["location_confidence"] = round(location_confidence, 4)
    if sources:
        copresence_block["sources"] = sources
    ...
    if copresence_block:
        out["copresence"] = copresence_block
    if graph_block:
        out["graph"] = graph_block
    if cohort_tags:
