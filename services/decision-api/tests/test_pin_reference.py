...
def test_pin_reference_then_identical_snapshot_zero_drift(temp_data_dir) -> None:
    ...
    assert (
        client.post(
            "/v1/calibration/reference/default",
            json=_snap(golden, "2026-09-15T00:00:00Z"),
        ).status_code
        == 200
    )
    assert (
        client.post(
            "/v1/calibration/snapshots", json=_snap(golden, "2026-09-15T01:00:00Z")
        ).status_code
        == 201
    )
...
def test_pin_reference_detects_shifted_distribution(temp_data_dir) -> None:
    ...
    golden = {"0.0-0.2": 70, "0.2-0.4": 20, "0.4-0.6": 10}
    client.post(
        "/v1/calibration/reference/default", json=_snap(golden, "2026-09-15T00:00:00Z")
    )
    shifted = {"0.0-0.2": 20, "0.2-0.4": 30, "0.4-0.6": 50}  # mass moved right
    client.post(
        "/v1/calibration/snapshots", json=_snap(shifted, "2026-09-15T02:00:00Z")
    )
    drift = compute_drift_for_tenant("acme", "default")
...
def test_pin_persists_reference_file(temp_data_dir) -> None:
    ...
    hist = {"allow": 9, "deny": 1}
    client.post(
        "/v1/calibration/reference/default", json=_snap(hist, "2026-09-15T00:00:00Z")
    )
