"""Shipped graph_v1 pack — FLAG on named hop etype, not a score blob."""
...
_PACK_PATH = (
    Path(__file__).resolve().parents[1] / "rules" / "graph_v1_uses_device_v1.json"
)
...
def test_pack_is_observe_seed_and_validates():
    ...
    assert data.get("name") == "graph_v1_uses_device_v1"
    assert validate_rule_pack(data) == []
    rule = data["rules"][0]
    assert rule["id"] == "uses_device_multi_or_sibling"
...
def test_empty_url_and_graph_missing_do_not_fire():
    ...
    assert delta2 == 0.0
    why2 = pack_why_from_hop(tagged.get("_graph_hop_v1"))
    assert why2["named"] == "graph:missing"
...
def test_sibling_prior_flag_alone_with_signed_etype_flags():
    ...
    why = pack_why_from_hop(hop)
    assert "has_etype:USES_DEVICE" in why["named"]
    assert "sibling_prior_flag" in why["named"]
    assert "has_multi_id" not in why["named"]
