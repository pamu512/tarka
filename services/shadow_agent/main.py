...
def _legacy_tarka_error_envelope(
    *,
    ...
    """Match ``services/shared/error_handlers._payload`` shape (Tarka API clients)."""
    return {
        "error": {
            "code": code,
            "message": message,
...
def build_app(
    *,
    ...
    async def http_hypothesis_backtest_blocks(
        ...
        try:
            from hypothesis_backtest_client import (
    ...
    async def http_validate_hypothesis_backtest(
        ...
        try:
            from hypothesis_backtest_client import validate_suggested_rule_for_analyst
