...
class OrchestratorSettings(BaseSettings):
    """
    Process env + optional ``.env`` / ``infra/deploy/.env``.
    ...
    hil_override_idempotency_ttl_sec: int = Field(
        default=3600,
        ge=1,
        validation_alias="HIL_OVERRIDE_IDEMPOTENCY_TTL_SEC",
    )
    hil_override_idempotency_redis_prefix: str = Field(
        default="tarka:hil_override:idempotency:",
        min_length=1,
        validation_alias="HIL_OVERRIDE_IDEMPOTENCY_REDIS_PREFIX",
    )
