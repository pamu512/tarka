"""Lite Redis Streams + Postgres analytics app (formerly services/data-platform).

Canonical home: ``data_plane.platform``. Port 8014 remains a one-release
compatibility listener via ``services/data-platform`` (thin re-export).

Removal gate: delete ``services/data-platform`` and the 8014 compose/Helm
bindings when no callers remain (rg -n '8014|data-platform' --glob '*.{yml,ts,tsx,py,md}').
"""
...
from data_plane.platform.analytics import ensure_schema, query_decisions, write_event
from data_plane.platform.config import settings
from data_plane.platform.streaming import ensure_group, parse_stream_event, publish_event
...
def _try_setup_observability(app: FastAPI) -> None:
    try:
        try:
            from tarka_shared.config_validation import log_runtime_warnings
        except ImportError:
            from config_validation import log_runtime_warnings  # type: ignore[no-redef]
        try:
            from tarka_shared.observability import setup_observability
    ...
    except Exception as exc:  # pragma: no cover - shared optional in unit smoke
        log.warning("platform observability setup skipped: %s", exc)
...
def create_platform_app(*, with_observability: bool = True) -> FastAPI:
    """Build the Redis+Postgres lite platform FastAPI app (port 8014 contract)."""
    application = FastAPI(
    ...
    if with_observability:
        _try_setup_observability(application)
    ...
    @application.post("/v1/events", dependencies=[Depends(require_api_key)])
    ...
    @application.post("/v1/events/batch", dependencies=[Depends(require_api_key)])
    ...
    @application.get("/v1/stream/info", dependencies=[Depends(require_api_key)])
    ...
    @application.get("/v1/analytics/decisions", dependencies=[Depends(require_api_key)])
...
# Default ASGI app for the 8014 compatibility listener.
