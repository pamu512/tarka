...
async def publish_profile(body: CalibrationProfileIn) -> dict[str, Any]:
    data = _load_profiles()
...
@router.get("/profiles/{tenant_id}/{profile_id}")
async def get_profile(tenant_id: str, profile_id: str) -> dict[str, Any]:
    return _active_profile_for(tenant_id, profile_id)


@router.post("/profiles/{tenant_id}/{profile_id}/activate")
async def activate_profile(tenant_id: str, profile_id: str, body: dict[str, int]) -> dict[str, Any]:
...
@router.get("/drift")
async def drift(tenant_id: str, profile_id: str = "default") -> dict[str, Any]:
    return {
        "tenant_id": tenant_id,
...
@router.get("/reliability/export")
async def reliability_export(tenant_id: str, profile_id: str = "default") -> dict[str, Any]:
...
@router.post("/score")
async def score(body: CalibrationScoreRequest, request: Request) -> dict[str, Any]:
    try:
    ...
    drift_meta = _drift_hint(body.tenant_id, body.profile_id)
    drift_penalty = 0.0
    ds = drift_meta.get("drift_score")
