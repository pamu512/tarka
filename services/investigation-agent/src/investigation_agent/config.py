...
class Settings(BaseSettings):
    ...
    copilot_hide_tools_without_upstream: bool = Field(
        default=True,
        description=(
            "If true, omit tool definitions for upstreams that are not configured (empty CASE_API_URL, DECISION_API_URL, or GRAPH_SERVICE_URL)."
        ),
    )
    ...
    copilot_high_impact_tools: str = Field(
        default=(
            "ingest_labeled_rows,run_replay_ab_comparison,screen_sanctions_pep,"
            "summarize_adverse_media,consolidate_entity_profile"
        ),
        description="Comma-separated tools blocked server-side unless allowlisted or reviewer secret matches.",
    )
    copilot_safe_action_allowlist: str = Field(
        default="",
        description="Comma-separated subset of high-impact tools permitted without reviewer secret.",
    )
