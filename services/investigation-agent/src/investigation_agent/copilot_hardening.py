...
def parse_high_impact_tools(raw: str) -> frozenset[str]:
    if not (raw or "").strip():
        return frozenset()
    return frozenset(x.strip() for x in raw.split(",") if x.strip())


def parse_safe_action_allowlist(raw: str) -> frozenset[str]:
    if not (raw or "").strip():
        return frozenset()
    return frozenset(x.strip() for x in raw.split(",") if x.strip())


def check_safe_action_gate(
    tool_name: str,
    *,
    high_impact_tools: frozenset[str],
    safe_action_allowlist: frozenset[str],
    reviewer_secret: str,
    reviewer_header: str,
) -> tuple[bool, str | None]:
    """
    Server-side gate for high-impact tool execution (Q2-E02).

    Returns ``(permitted, error_code)``.
    """
    name = (tool_name or "").strip()
    if not name or name not in high_impact_tools:
        return True, None
    secret = (reviewer_secret or "").strip()
    header = (reviewer_header or "").strip()
    if secret and header and secrets.compare_digest(header, secret):
        return True, None
    if name in safe_action_allowlist:
        return True, None
    return False, "safe_action_blocked:high_impact_tool_not_allowlisted"
