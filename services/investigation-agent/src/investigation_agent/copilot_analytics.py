...
def schedule_citation_verifier_outcome(
    settings: Settings,
    *,
    tenant_id: str,
    analyst_id: str,
    ...
    """Persist citation verifier rollup to the analytics sink (Q2-E02)."""
    payload = _base_payload(settings, tenant_id, analyst_id)
    p = (persona or "investigation").strip().lower()
    if p not in ("investigation", "orchestrator"):
        p = "investigation"
    ...
    schedule_emit(settings, "copilot.citation.verifier", payload)
