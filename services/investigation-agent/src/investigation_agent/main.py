...
from investigation_agent.copilot_hardening import (
    ...
    parse_high_impact_tools,
    parse_safe_action_allowlist,
...
from investigation_agent.store_backend import store_config_errors, store_mode
...
async def _execute_tool(
    ...
    *,
    ...
    permitted, gate_err = check_safe_action_gate(
        name,
        high_impact_tools=parse_high_impact_tools(settings.copilot_high_impact_tools),
        safe_action_allowlist=parse_safe_action_allowlist(settings.copilot_safe_action_allowlist),
        reviewer_secret=(settings.copilot_reviewer_secret or ""),
    ...
    if not permitted:
        return normalize_tool_error_shape(
            name,
            {
                "error": gate_err or "safe_action_blocked",
                "detail": gate_err or "high-impact tool blocked by server allowlist",
                "code": gate_err or "safe_action_blocked",
                "severity": "error",
                "retryable": False,
            },
        )
    ...
    elif name == "get_entity_tags" or name == "get_entity_velocity":
...
async def _build_chat_response(body: ChatRequest, request: Request) -> dict[str, Any]:
    ...
    if not settings.openai_api_key:
        ...
        copilot_analytics.schedule_citation_verifier_outcome(
            ...
            analyst_id=body.analyst_id,
            turn_id=turn_id,
            verifier_summary=verifier_summary.model_dump(mode="json"),
            persona=body.persona,
        )
        copilot_analytics.schedule_turn_completed(
            settings,
