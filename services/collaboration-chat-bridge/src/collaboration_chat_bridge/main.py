...
from collaboration_chat_bridge.trusted_scope import trusted_scope_headers
...
_CHAT_BRIDGE_SUBAPP = os.environ.get("TARKA_CHAT_BRIDGE_SUBAPP", "").strip() == "1"
if _CHAT_BRIDGE_SUBAPP:
    # Empty URL → in-process chat against the mounting investigation-agent app.
    os.environ.setdefault("INVESTIGATION_AGENT_URL", "")


def _bp(path: str) -> str:
    """Standalone serves ``/v1/...``; when mounted under ``/collab`` serve ``/...``."""
    if not path.startswith("/"):
        path = "/" + path
    return path if _CHAT_BRIDGE_SUBAPP else f"/v1{path}"
...
@app.post(_bp("/thread-correlations"))
...
@app.get(_bp("/thread-correlations/{platform}/{workspace_id}/{thread_key}"))
...
@app.get(_bp("/outbound/case-state-change/schema"))
async def case_state_change_schema():
    return CaseStateChangeWebhookPayload.model_json_schema()


@app.post(_bp("/outbound/case-state-change/fixture"))
