...
**Canonical package:** `collaboration_chat_bridge` under `src/`. Investigation-agent mounts the same app at **`/collab`** (`TARKA_CHAT_BRIDGE_SUBAPP=1`); there is no duplicate implementation under `investigation_agent.chat_bridge` (shim only).
