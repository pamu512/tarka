...
*   **JSON rule engine (Python):** `services/decision-api` evaluates versioned JSON rule packs in-process with strict validation for optional **AND/OR AST** payloads (`when_ast` on rules) and the same leaf operators as the legacy flat `when` lists. The historical [`services/rule-engine/`](services/rule-engine/) Rust crate remains in-repo as reference material but is **not** required for builds or CI.
