...
| Commercial install pack / support claims | [`SUPPORT.md`](../../SUPPORT.md) — VPC / Helm / SSO / pack-GitOps assist + severity **intent**. Export how-to is the Pack GitOps export assist section, not a new SKU. Not a 99.99% SLA. |
...
| Observe ≠ live until promote gates pass. Ungated → human Promote. Gates defined+met → may auto-Promote (default off). Propose→Confirm Demote human-only; auto-demote forbidden. Model never Promotes or demotes. Empty URL / model never Promotes or demotes. | Live hop FLAG without Promote; always-on Day-1 auto-Promote; auto-demote |
...
| Enforcement contract-gated; default emit-only ([enforcement-v1](../contracts/enforcement-v1.md)). Outbound decision/action webhooks HMAC-SHA256 signed (`x-tarka-signature`) when secret is set. Empty URL = plane off. Suggested-action `action_id` is hex SHA-256 of tenant + trace_id + action token + pack hash — stable across retries, not a random UUID per POST. Inbound product ACK binds delivery status to `trace_id` + `action_id` (queryable). ACK is not Promote/Demote and not a case inbox. Desk `/decisions/:id` delivery-status glass shows emitted / acked / failed / not configured. Empty URL = not configured (plane off). emit_only copy is advisory emit, not a payout block. | Handoff as Day-1 default; silent block in emit-only; unsigned enforcement webhooks as the contract; random `action_id` per POST; ACK as Promote/Demote; case CRM from ACK; delivery glass as D9 retry/DLQ suite or case queue; “we blocked payout” in emit_only |
| Desk delivery-status glass (G4.4) next to pack-why: emit/ACK chips only. D9.4 may extend the same strip with retry/DLQ — do not fork a second log product. | Case queue / CRM from delivery chips; brochure enforcement suite; ACK as Promote/Demote; executing holds from this glass |
...
| Retry POSTs reuse the G4.2 `action_id` (hex SHA-256 of `tarka.action_id/v1` + tenant + `trace_id` + action token + pack hash) as `idempotency_key`. Journal rows share that key; in-process dedupe returns one logical delivery with `attempt_count`. Buyer sinks apply once. Not a second competing `delivery_id`. | Second competing `action_id` / `delivery_id`; silent block/deny on duplicate; Tarka executing holds/payouts; CRM tickets as the action sink; D9.4 desk glass |
| Delivery journal queryable by `trace_id` (`GET /v1/enforcement/deliveries?trace_id=&tenant_id=&action_id=&status=`). Tenant-scoped. Empty URL / `skipped`+`webhook_unset` → `not_configured`. Journal HTTP 2xx → `emitted` (not product ACK). Product ACK → `acked`. Reuses `logical_enforcement_delivery`. Ops tail stays. Not a case CRM. | Case CRM / assignee / SAR / tickets from this GET; journal status as Promote/Demote; cross-tenant god-view; D9.4 desk glass as shipped; silent block; invented rows for an unknown trace |
