...
| L2 | Point-in-time online serve (`GET /v1/features/{entity_type}/{entity_id}?as_of=`) | On only when `FEATURE_STORE_URL` is set. Empty = off. Evaluate fail-soft on miss/timeout (not 500). L2 does not decide ALLOW/DENY/Promote/demote. |
