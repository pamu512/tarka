...
| `investigation` | `case-api`, `investigation-agent`, `graphql-gateway`, `frontend`, `integration-ingress` | `graph-service` (Slack/Teams/Lark ingress is **embedded** on `investigation-agent` **`/v1/chat/…`**) | Postgres, Redis, optional graph store          | Analyst workflows and evidence reviews           |
...
| `full`          | all above bundles                                                                       | risk services (`counter-service`, `location-service`; optional **external** calibration via `CALIBRATION_SERVICE_URL`) | Postgres, Redis, graph store, NATS, ClickHouse | Platform-wide multi-team fraud operations        |
...
| `investigation` | `cases`, `agent`, `gateway`, `integration`, optional `graph` | `caseApi.enabled`, `investigationAgent.enabled` (includes embedded `chat_bridge` on **`/v1/chat/…`**), `graphqlGateway.enabled`, `integrationIngress.enabled`, optional `graphService.enabled` |
