...
Use the `prod-on-k8s` overlay (managed PG/Redis required, in-cluster stores off, PDB + HPA on evaluate, `allowInsecureNoAuth: false`, `tenantBindingRequired: true`). Investigation-agent is **on** with `dataPersistence.mode=postgres` and `replicaCount: 2` (same external `databaseUrl` as core-api). `local-sqlite` still forbids `replicaCount > 1`. CI (`infra/scripts/ci/helm_prod_honesty.sh`, `cloud-preset-smoke`) fails if a production-labeled Helm preset would run decisions/audit on sqlite, put decision/audit/pack/label data on `emptyDir`, or enable in-cluster Postgres for core. That is a no-sqlite-prod honesty gate, not a GitLab-grade complete install. See `docs/contracts/production-install-v1.md` when present (G0).
