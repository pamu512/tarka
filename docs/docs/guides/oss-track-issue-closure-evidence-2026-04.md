...
| **#38** | Community vs Pro deployment docs      | **PR #93**                                                          | `docs/docs/guides/deployment-profiles-community-vs-pro.md`, `infra/deploy/env/*.env.example`, links from `infra/deploy/.env.example` + `docs/docs/index.md`                                                             |
...
| Runtime resilience + auth hardening | **Implementation landed (branch evidence pending merge SHA)** | Per-upstream circuits and fallback reasons (`circuit_calibration/counter/location`) in decision runtime; fail-closed shared auth in `services/shared/auth.py`, RBAC posture in `services/shared/auth_rbac.py`; SLO endpoints in each new service and burn-rate coverage extended in `infra/deploy/observability/prometheus-rules/slo-burn.yml`, with scrape targets wired in `infra/deploy/observability/prometheus.yml`. |
