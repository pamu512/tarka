...
3. **Labels** — `y_label` + `why` from the existing y_label store (analyst override). Late chargeback sits on the same record as two fields: raw `dispute_outcome` and `chargeback_class` (`FRAUD` / `FRIENDLY` / `SERVICE` / `UNKNOWN`). A signed webhook `POST /v1/webhooks/late-label` binds those fields to the original evaluate receipt by `trace_id`, `evaluation_token`, or `decision_token`. Join keys and bind order: [`label-join-v1.md`](../../../contracts/label-join-v1.md). It does not reconstruct features. No snapshot → label is still recorded, `trainable: false`. Outcome other than `FRAUD` is still a label. There is no chargeback inbox or case CRM here.
