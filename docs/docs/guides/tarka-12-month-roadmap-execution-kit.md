...
### Q1/Q2 closure status (2026-06-19)

All Q1 and Q2 umbrella epics **#127–#142** are closed. Evidence paths and CI job names: [github-q1-q2-epic-closure-evidence-jun-2026.md](github-q1-q2-epic-closure-evidence-jun-2026.md).

| Epic ID | Closed |
| ------- | ------ |
| Q1-E01–Q1-E08 | 2026-06-19 |
| Q2-E01–Q2-E08 | 2026-06-19 |
...
| ------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| Q1-D01 | Cloud preset promotion baseline         | `.github/workflows/ci.yml`, `infra/scripts/deploy/generate_cloud_values.py`, `infra/scripts/ci/cloud_preset_smoke.py`, `infra/deploy/helm/fraud-stack/presets/*.yaml`                                      | Platform Engineering    | SRE, QA                                    | Preset generation and validation pass in CI and are consumed by staging release playbooks.           |
| Q1-D02 | Helm service surface contract hardening | `infra/deploy/helm/fraud-stack/templates/{calibration-service,counter-service,location-service,opa,collaboration-chat-bridge,workload-operations}.yaml`, `infra/deploy/helm/fraud-stack/values.yaml` | Platform Engineering    | Service Owners                             | Enabled chart profiles render with complete env/dependency contracts and no template drift.          |
...
| Q1-D05 | SLO burn coverage expansion             | `infra/deploy/observability/prometheus-rules/slo-burn.yml`                                                                                                                                     | SRE                     | Service Owners                             | Burn-rate rules include new risk services with runbook links and severity mapping.                   |
...
| Q1-D07 | Production-hardening env parity         | `infra/deploy/.env.example`, `infra/deploy/docker-compose.yml`, `infra/deploy/docker-compose.production-hardening.yml`, `infra/deploy/hosted/k8s/overlays/*/kustomization.yaml`                                  | DevEx                   | Platform Engineering, Security Engineering | Environment contracts are aligned across local, hosted-k8s, and production-hardening paths.          |
