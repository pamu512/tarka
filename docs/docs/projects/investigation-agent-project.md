...
**Distribution:** The **canonical agent implementation** is **`services/investigation-agent`** in **this repo** (codename **Saarthi**). The **standalone container** for the same code is **[`distributions/saarthi-pro-agent`](../../../distributions/saarthi-pro-agent/README.md)** (CI: `docker-build-saarthi-pro-agent`). **[Saarthi Pro](https://github.com/pamu512/Saarthi-pro)** is the **commercial** umbrella—support, registry publishing, and customer-facing releases **pin** a fraud-stack commit / image digest (see [distribution & contract parity](../guides/saarthi-pro-distribution-and-contract-parity.md)); the Pro repo does not replace the OSS source of truth.
