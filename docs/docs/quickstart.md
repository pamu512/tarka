...
This sets `FEATURE_SERVICE_URL`, `ML_SCORING_URL`, and `GRAPH_SERVICE_URL` for **decision-api** (see `infra/deploy/.env.lite-oss.example`). With feature-service up, open **Governance → Feature tools** in the UI (`/ops/features`) to query velocity counters and run parity verify against Redis.
