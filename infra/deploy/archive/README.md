...
| `docker-compose.graph-wire.yml` | Janus overlay (AGE is already on lite) |

## Canonical compose (stay under `infra/deploy/`)

1. **Day-1:** `docker-compose.lite.yml` (optional fraud-desk).
2. **Local modular / profiles:** `docker-compose.yml`
