...
```bash
# United States build
docker compose -f infra/deploy/docker-compose.yml -f infra/deploy/profiles/ai-governance/docker-compose.override-us.yml --profile full up -d --build

# EU / UK build
docker compose -f infra/deploy/docker-compose.yml -f infra/deploy/profiles/ai-governance/docker-compose.override-eu-uk.yml --profile full up -d --build

# Global (explicit; same as omitting override if defaults unchanged)
docker compose -f infra/deploy/docker-compose.yml -f infra/deploy/profiles/ai-governance/docker-compose.override-global.yml --profile full up -d --build
```
...
Copy variables from `us.env`, `eu-uk.env`, or `global.env` into `infra/deploy/.env` or your secret store.
...
```bash
helm upgrade --install tarka ./infra/deploy/helm/fraud-stack -f infra/deploy/profiles/ai-governance/helm/values-us.yaml
```
