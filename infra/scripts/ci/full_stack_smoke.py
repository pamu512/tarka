...
"""
Bring up infra/deploy/docker-compose.yml with --profile full, wait for HTTP health, POST evaluate, tear down.

Used by GitHub Actions; runnable locally from repo root:
  python infra/scripts/ci/full_stack_smoke.py
  python infra/scripts/ci/full_stack_smoke.py --skip-up   # stack already running (no compose down)
...
JSON_HEALTH = [
    ...
    ("orchestrator", "http://127.0.0.1:8790/health/full"),
]
