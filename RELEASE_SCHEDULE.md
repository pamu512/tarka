...
### `v1.2.0` — ship hold until Epic C RC gates (active)

**Do not** publish a **`v1.2.0`** Git tag or GitHub release, and do not treat the train as “shipped to users,” until **[Epic C Release Candidate Gate Criteria](docs/docs/guides/counter-replay-parity.md#epic-c-release-candidate-gate-criteria)** are satisfied on the release candidate commit (operational validation only; staging `AGG_KEY_VERSION` cutover log required). Integration branches (e.g. `release/v1.2.0`) may continue to move for development; the hold is on **customer-facing release finalization** only.
