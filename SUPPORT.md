...
| **SSO wiring** | Wire OIDC for **desk humans** (`OIDC_ISSUER` / `OIDC_JWKS_URL` / `OIDC_AUDIENCE`; `OIDC_CLIENT_SECRET` on `global.appSecretsName`). Empty issuer stays local / API-key mode. | Become the IdP. Replace machine `API_KEYS` with OIDC. Require OIDC to boot evaluate. |
...
OIDC is optional. API keys stay the machine / evaluate path. Production + a non-empty issuer requires resolved Redis (no in-process OIDC state fallback). See [deployment.md](docs/docs/guides/deployment.md).
