...
def test_verify_and_lock_event_acquires_with_set_nx_ex() -> None:
    async def _run() -> None:
        from orchestrator.middleware.idempotency import verify_and_lock_event
...
def test_release_lock_deletes_key() -> None:
    async def _run() -> None:
        from orchestrator.middleware.idempotency import release_lock
...
def test_verify_and_lock_rejects_empty_key() -> None:
    async def _run() -> None:
        from orchestrator.middleware.idempotency import IdempotencyKeyError, verify_and_lock_event
