"""Shared operational signal persistence (legacy feedback bridges)."""
...
from orchestrator.middleware.idempotency import release_lock, verify_and_lock_event
...
from orchestrator.services.operational_signal_persist import (
...
def require_redis_client(request: Request) -> object:
...
def require_session_factory(request: Request) -> async_sessionmaker[AsyncSession]:
...
async def fetch_existing_event_id(
...
async def submit_operational_signal(
    request: Request,
    body: OperationalSignalCreate,
) -> OperationalSignalAcceptedResponse:
    """Persist one operational signal with Redis idempotency locking (legacy bridge path)."""
    redis_client = require_redis_client(request)
    session_factory = require_session_factory(request)

    redis_key = idempotency_redis_key(body.idempotency_key)
    ...
    if not lock_acquired:
        existing_id = await fetch_existing_event_id(session_factory, body.idempotency_key)
    ...
    logger.info(
        "operational_signal_accepted event_id=%s signal_type=%s entity_id=%s",
        event_id,
