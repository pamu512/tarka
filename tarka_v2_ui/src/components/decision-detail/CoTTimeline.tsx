...
export function CoTTimeline({ steps }: CoTTimelineProps) {
  const visible = steps.filter(
    (step) =>
      (typeof step.heading === "string" && step.heading.trim().length > 0) ||
      (typeof step.body === "string" && step.body.trim().length > 0),
  );
        ...
        {visible.map((step, i) => {
          const heading =
            typeof step.heading === "string" && step.heading.trim()
              ? step.heading.trim()
              : `Step ${i + 1}`;
          const body = typeof step.body === "string" ? step.body : String(step.body ?? "");

          return (
            <li
              key={`${step.stepIndex ?? i}-${i}`}
