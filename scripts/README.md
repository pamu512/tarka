...
| Script | Purpose |
|--------|---------|
| [`ci/validate_openapi_yaml.py`](ci/validate_openapi_yaml.py) | Parse-check every `contracts/openapi/*.yaml` with PyYAML (same step as `.github/workflows/ci.yml` **lint**). Run: `pip install pyyaml && python infra/scripts/ci/validate_openapi_yaml.py` |
...
| [`github/create_v130_wave1_q1_child_issues.py`](github/create_v130_wave1_q1_child_issues.py) | Create **v1.3.0 Wave 1** issues for Q1-E04 / Q1-E08 (`wave-1`, `reliability-gate` labels; AC + Reliability Gate evidence + PR placeholders; `--dry-run` first). |
